...
open class SwiftAddressBookPerson : SwiftAddressBookRecord {
	...
	open class func createInSourceWithVCard(_ source : SwiftAddressBookSource, vCard : String) -> [SwiftAddressBookPerson]? {
		...
		if let persons = abPersons {
			for person : ABRecord in persons as [AnyObject] {
	...
	open class func createVCard(_ people : [SwiftAddressBookPerson]) -> String {
		...
		let data : Data = ABPersonCreateVCardRepresentationWithPeople(peopleArray as CFArray!).takeRetainedValue() as Data
	...
	open var dates : Array<MultivalueEntry<AnyObject>>? {
		get {
	...
	fileprivate func setSingleValueProperty<T : Any>(_ key : ABPropertyID,_ value : T?) {
		ABRecordSetValue(self.internalRecord, key, value as CFTypeRef!, nil)
	}
